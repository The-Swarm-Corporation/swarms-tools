...
async def test_fetch_token_by_mint_address_async():
    ...
    with patch('swarms_tools.finance.jupiter_tools.get_aiohttp_session') as mock_get_session:
        mock_get_session.return_value = mock_session
        mock_session.json.return_value = SAMPLE_TOKEN
        
        result = await jupiter_fetch_token_by_mint_address_async(mint_address)
...
def test_fetch_token_by_mint_address():
    ...
    with patch('swarms_tools.finance.jupiter_tools.jupiter_fetch_token_by_mint_address_async') as mock_async:
...
async def test_get_token_prices_async():
    ...
    with patch('swarms_tools.finance.jupiter_tools.get_aiohttp_session') as mock_get_session:
        ...
        result = await jupiter_get_token_prices_async(token_ids, vs_token="USDC")
        assert result == SAMPLE_PRICES
...
async def test_get_token_prices_invalid_params():
    ...
    token_ids = ["So11111111111111111111111111111111111111112"]
    try:
        await jupiter_get_token_prices_async(token_ids, vs_token="USDC", show_extra_info=False)
...
def test_get_token_prices():
    ...
    with patch('swarms_tools.finance.jupiter_tools.jupiter_get_token_prices_async') as mock_async:
...
def test_fetch_tradable_tokens():
    ...
    with patch('swarms_tools.finance.jupiter_tools.jupiter_fetch_tradable_tokens_async') as mock_async:
...
async def test_api_error_handling():
    ...
    with patch('swarms_tools.finance.jupiter_tools.get_aiohttp_session') as mock_get_session:
        mock_get_session.return_value = mock_session
        mock_session.raise_for_status.side_effect = aiohttp.ClientError()
        
        try:
            await jupiter_fetch_token_by_mint_address_async(mint_address)
            raise AssertionError("Expected ClientError was not raised")
